...
typedef int (*command_handler_t)(list_wrap_t **, bool, int,
    char **);

static int
ch_add_string(list_wrap_t **lw, bool array, int argc, char **argv)
{
...
static int
ch_add_boolean(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];
...
static int
ch_add_boolean_value(list_wrap_t **lw, bool array, int argc, char **argv)
{
	int i;
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];
	bool arrval[MAX_ARGS];
...
static int
ch_add_byte(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(u_char, byte, 0, UCHAR_MAX, uint64_t, parse_uint)
}

static int
ch_add_int8(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(int8_t, int8, INT8_MIN, INT8_MAX, int64_t, parse_int)
}

static int
ch_add_uint8(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(uint8_t, uint8, 0, UINT8_MAX, uint64_t, parse_uint)
}

static int
ch_add_int16(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(int16_t, int16, INT16_MIN, INT16_MAX, int64_t, parse_int)
}

static int
ch_add_uint16(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(uint16_t, uint16, 0, UINT16_MAX, uint64_t, parse_uint)
}

static int
ch_add_int32(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(int32_t, int32, INT32_MIN, INT32_MAX, int64_t, parse_int)
}

static int
ch_add_uint32(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(uint32_t, uint32, 0, UINT32_MAX, uint64_t, parse_uint)
}

static int
ch_add_int64(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(int64_t, int64, INT64_MIN, INT64_MAX, int64_t, parse_int)
}

static int
ch_add_uint64(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];

	DO_CMD_NUMBER(uint64_t, uint64, 0, UINT64_MAX, uint64_t, parse_uint)
}

static int
ch_add_double(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *nvl = (*lw)->lw_nvl[(*lw)->lw_pos];
	double val;
...
static int
ch_end(list_wrap_t **lw, bool array, int argc, char **argv)
{
	nvlist_t *parent;
	char *name;
...
static int
ch_next(list_wrap_t **lw, bool array, int argc, char **argv)
{
	if (!(*lw)->lw_array) {
...
static int
ch_add_object(list_wrap_t **lw, bool array, int argc, char **argv)
{
...
command_t command_handlers[] = {
	{ "add_boolean", ch_add_boolean, 1, 1, false },
	{ "add_boolean_value", ch_add_boolean_value, 2, 2, false },
	{ "add_byte", ch_add_byte, 2, 2, false },
	{ "add_int8", ch_add_int8, 2, 2, false },
	{ "add_uint8", ch_add_uint8, 2, 2, false },
	{ "add_int16", ch_add_int16, 2, 2, false },
	{ "add_uint16", ch_add_uint16, 2, 2, false },
	{ "add_int32", ch_add_int32, 2, 2, false },
	{ "add_uint32", ch_add_uint32, 2, 2, false },
	{ "add_int64", ch_add_int64, 2, 2, false },
	{ "add_uint64", ch_add_uint64, 2, 2, false },
	{ "add_double", ch_add_double, 2, 2, false },
	{ "add_string", ch_add_string, 2, 2, false },
	{ "add_object", ch_add_object, 1, 1, false },
	{ "add_boolean_array", ch_add_boolean_value, 1, MAX_ARGS, true },
	{ "add_byte_array", ch_add_byte, 1, MAX_ARGS, true },
	{ "add_int8_array", ch_add_int8, 1, MAX_ARGS, true },
	{ "add_uint8_array", ch_add_uint8, 1, MAX_ARGS, true },
	{ "add_int16_array", ch_add_int16, 1, MAX_ARGS, true },
	{ "add_uint16_array", ch_add_uint16, 1, MAX_ARGS, true },
	{ "add_int32_array", ch_add_int32, 1, MAX_ARGS, true },
	{ "add_uint32_array", ch_add_uint32, 1, MAX_ARGS, true },
	{ "add_int64_array", ch_add_int64, 1, MAX_ARGS, true },
	{ "add_uint64_array", ch_add_uint64, 1, MAX_ARGS, true },
	{ "add_string_array", ch_add_string, 1, MAX_ARGS, true },
	{ "add_object_array", ch_add_object, 1, 1, true },
	{ "end", ch_end, 0, 0, false },
	{ "next", ch_next, 0, 0, false },
	{ {0} }
};
